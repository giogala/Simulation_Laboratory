...
public:
    ...
    vec& operator[](int i) {return _mp[i]};
    int& operator()(int i) {return GetG(i);};
    int& GetG(int i);
    int N();
    bool Check();
    void Distances();
    ...
    vector <int> Cut(int i, int j=-1);
    vector <int> Cut(vector <int> a,int i, int j=-1);
    void Rebuild(vector <int> r,int i = 1);
    void Rebuild(vector <int> zip,vector <int> r,int i = 1);
...
public:
    ...
    void Sort(vector <int>& guy, vector <int>& dad);
    void Sort();
